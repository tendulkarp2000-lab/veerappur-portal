...
function App() {
    ...
    useEffect(() => {
        ...
        // Automatic polling for new requests/updates
        const pollInterval = setInterval(() => {
            fetch('/api/db')
                ...
                .then(data => {
                    setDb(prev => {
                        if (!prev) return data;
                        // Check if critical arrays or values have changed to minimize re-renders
                        const bookingsChanged = JSON.stringify(prev.bookings) !== JSON.stringify(data.bookings);
                        const noticesChanged = JSON.stringify(prev.notices) !== JSON.stringify(data.notices);
                        const festivalsChanged = JSON.stringify(prev.festivals) !== JSON.stringify(data.festivals);
                        const galleryChanged = JSON.stringify(prev.gallery) !== JSON.stringify(data.gallery);
                        
                        if (bookingsChanged || noticesChanged || festivalsChanged || galleryChanged || prev.visitorCount !== data.visitorCount) {
                            return data;
                        }
                        return prev;
                    });
                })
                .catch(err => console.warn("Polling fetch failed:", err));
        }, 10000); // 10 seconds
                ...
                <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="text-xs text-gold/70 flex items-center gap-1.5 font-tamil select-all">
                                <Icon name="mail" className="w-4 h-4 text-gold" /> veerappurtempleofficial@gmail.com
                            </div>
                        </div>
                    </div>
                </div>
...
function ServicesPage({ t, language, db, saveState }) {
    ...
    const handleSubmitRequest = (e, requestType) => {
        ...
        if (!name || name.trim().length < 3) {
            alert(language === 'ta' ? "பெயர் குறைந்தது 3 எழுத்துக்களைக் கொண்டிருக்க வேண்டும்." : "Name must be at least 3 characters.");
            return;
        }
        ...
        if (!phone || !phoneRegex.test(phone)) {
            alert(language === 'ta' ? "சரியான 10 இலக்க போன் எண்ணை உள்ளிடவும் (e.g. 9876543210)." : "Please enter a valid 10-digit mobile number starting with 6-9.");
            return;
        }

        // Captcha validation
        if (parseInt(captchaInput) !== (captchaNum1 + captchaNum2)) {
            alert(language === 'ta' ? "பாதுகாப்பு விடை தவறானது. மீண்டும் முயற்சிக்கவும்." : "Incorrect security response. Please try again.");
            return;
        }

        if (!date) {
            alert(language === 'ta' ? "தேதி கட்டாயம் நிரப்பப்பட வேண்டும்." : "Date is required.");
            return;
        }
        ...
        if (requestType === 'room') {
            ...
            details = { guideLanguage: guideLanguage || "Tamil" };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-5 bg-stone-light border border-gold/30 p-6 rounded-lg shadow-gold">
                    ...
                    <form onSubmit={(e) => handleSubmitRequest(e, tab === 'rooms' ? 'room' : tab === 'mandapam' ? 'mandapam' : tab === 'offerings' ? 'offering' : tab === 'transport' ? 'transport' : tab === 'guide' ? 'guide' : 'prasatham')} className="space-y-4 text-xs sm:text-sm">
                        ...
                        <div>
                            <label className="block text-slate-300 font-tamil text-xs mb-1">
                                {language === 'ta' ? `பாதுகாப்பு கேள்வி: ${captchaNum1} + ${captchaNum2} = ? *` : `Security Question: ${captchaNum1} + ${captchaNum2} = ? *`}
                            </label>
                            <input
                                type="number"
                                required
                                ...
                                className="w-full bg-stone border border-gold/30 rounded p-2.5 text-slate-200 focus:outline-none focus:border-gold"
                                placeholder={language === 'ta' ? "பாதுகாப்பு விடையை உள்ளிடவும்" : "Enter security response"}
                            />
                        </div>
...
function HelpDeskPage({ t, language, db, saveState }) {
    ...
    // Math Captcha state for HelpDesk
    const [captchaNum1, setCaptchaNum1] = useState(Math.floor(Math.random() * 9) + 1);
    const [captchaNum2, setCaptchaNum2] = useState(Math.floor(Math.random() * 9) + 1);
    const [captchaInput, setCaptchaInput] = useState("");
            ...
            <div className="bg-stone-light border border-gold/30 p-6 sm:p-8 rounded-lg shadow-gold">
                {subTab === 'complaint' ? (
                    <form onSubmit={handleComplaintSubmit} className="space-y-5 text-xs sm:text-sm">
                        ...
                        <div>
                            <label className="block text-slate-300 font-tamil text-xs mb-1.5">
                                {language === 'ta' ? `பாதுகாப்பு கேள்வி: ${captchaNum1} + ${captchaNum2} = ? *` : `Security Question: ${captchaNum1} + ${captchaNum2} = ? *`}
                            </label>
                            <input
                                type="number"
